...
func mountCgroupV2(m *configs.Mount, c *mountConfig) error {
	...
	if err := mount(m.Source, dest, "", "cgroup2", uintptr(m.Flags), m.Data); err != nil {
		...
		if errors.Is(err, unix.EPERM) || errors.Is(err, unix.EBUSY) {
			...
			err = mount(src, dest, "", "", uintptr(m.Flags)|unix.MS_BIND, "")
...
func doTmpfsCopyUp(m *configs.Mount, rootfs, mountLabel string) (Err error) {
	...
	if err := mount(tmpDir, dest, "", "", unix.MS_MOVE, ""); err != nil {
...
func remount(m *configs.Mount, rootfs string, mountFd *int) error {
	...
	err := mount(source, dest, "", m.Device, flags, "")
	if err == nil {
		return nil
	}
	...
	return mount(source, dest, "", m.Device, flags, "")
}
...
func mountPropagate(m *configs.Mount, rootfs string, mountLabel string, mountFd *int) error {
	...
	for _, pflag := range m.PropagationFlags {
		if err := mount("", dest, "", "", uintptr(pflag), ""); err != nil {
			return err
		}
	}
	return nil
